...
public abstract class StudentReportRunBaseReader implements ItemReader<SchoolStudentCredentialDistribution> {

    private static final Logger LOGGER = LoggerFactory.getLogger(StudentReportRunBaseReader.class);
    ...
    protected void aggregate(String summaryContextName) {
        StudentReportSummaryDTO totalSummaryDTO = (StudentReportSummaryDTO)jobExecution.getExecutionContext().get(summaryContextName);
        if (totalSummaryDTO == null) {
            totalSummaryDTO = new StudentReportSummaryDTO();
